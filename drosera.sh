...
# Public fallback RPC (will be used if user's RPC invalid/missing)
PUBLIC_FALLBACK_RPC="${PUBLIC_FALLBACK_RPC:-https://ethereum-hoodi-rpc.publicnode.com}"
...
# Drosera home/binary (ABSOLUTE LOCK)
DRO_HOME="${DRO_HOME:-/root/.drosera}"
DRO_BIN_DIR="${DRO_BIN_DIR:-$DRO_HOME/bin}"
DROSERA_BIN="${DROSERA_BIN:-$DRO_BIN_DIR/drosera}"
...
select_rpc() {
  ...
  if [[ -n "$USER_RPC" ]]; then
    ...
    if probe_rpc "$rpc"; then
      ...
      warn "RPC bạn truyền qua --rpc không hợp lệ hoặc khác chain-id $CHAIN_ID → dùng RPC public mặc định."
      NEED_WARN_PUBLIC=1
      echo "$PUBLIC_FALLBACK_RPC"; return 0
    fi
  fi

  # No --rpc → force prompt once
  read -rp "Nhập RPC URL của bạn (bắt buộc): " rpc
  if [[ -z "$rpc" ]] || ! probe_rpc "$rpc"; then
    warn "RPC bạn nhập rỗng/không hợp lệ → dùng RPC public mặc định."
    NEED_WARN_PUBLIC=1
    echo "$PUBLIC_FALLBACK_RPC"; return 0
  fi
  echo "$rpc"; return 0
...
HOODI_RPC="$(select_rpc)"
export HOODI_RPC
export ETH_RPC_URL="$HOODI_RPC"
export ETH_BACKUP_RPC_URL="$HOODI_RPC"   # no dedicated backup in this mode

msg "RPC sử dụng: $HOODI_RPC"
if [[ "$NEED_WARN_PUBLIC" -eq 1 ]]; then
  warn "Bạn đang dùng RPC PUBLIC mặc định (${PUBLIC_FALLBACK_RPC}). Lưu ý: RPC public có thể **không ổn định** (rate-limit/403/độ trễ cao). Khuyến nghị chạy lại với --rpc <RPC_cá_nhân>."
fi

# =================== droseraup (installer) with fallback ===================
install_droseraup(){
...
# docker-compose.yaml (pin image, use selected RPC)
...
services:
  drosera-operator:
    ...
    environment:
      ...
      - DRO__ETH__BACKUP_RPC_URL=${ETH_BACKUP_RPC_URL:-${HOODI_RPC}}
...
# Safe wrappers for docker run (avoid triggering ERR trap)
safe_run() {
  # usage: safe_run <logfile> -- <cmd...>
  local logfile="$1"; shift
  [[ "$1" == "--" ]] && shift
  local rc=0
  if ! "$@" >"$logfile" 2>&1; then rc=$?; fi
  echo "$rc"
}
...
if [[ "$REGISTER_OK" -ne 1 || -n "$FORCE_REGISTER" ]]; then
  ...
  if [[ "$HAS_OP_PK_FLAG" -eq 1 ]]; then
    rc="$(safe_run "$reg_tmp" -- docker run --rm ghcr.io/drosera-network/drosera-operator:v1.20.0 \
      register --eth-chain-id "$CHAIN_ID" --eth-rpc-url "$HOODI_RPC" \
               --drosera-address "$DROSERA_ADDRESS" \
               --eth-private-key "$PK_0X")"
  else
    rc="$(safe_run "$reg_tmp" -- docker run --rm -e DRO__ETH__PRIVATE_KEY="$PK_0X" ghcr.io/drosera-network/drosera-operator:v1.20.0 \
...
if [[ "$OPTIN_OK" -ne 1 || -n "$FORCE_OPTIN" ]]; then
  if [[ "$TRAP_ADDR" =~ ^0x[0-9a-fA-F]{40}$ ]]; then
    ...
    run_optin() {
      ...
      tmp="$(mktemp)"
      if [[ "$HAS_OP_PK_FLAG" -eq 1 ]]; then
        rc="$(safe_run "$tmp" -- docker run --rm ghcr.io/drosera-network/drosera-operator:v1.20.0 \
              optin --eth-rpc-url "$rpc" \
                    --trap-config-address "$TRAP_ADDR" \
                    --eth-private-key "$PK_0X")"
      else
        rc="$(safe_run "$tmp" -- docker run --rm -e DRO__ETH__PRIVATE_KEY="$PK_0X" ghcr.io/drosera-network/drosera-operator:v1.20.0 \
